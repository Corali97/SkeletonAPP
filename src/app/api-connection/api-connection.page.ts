...
export class ApiConnectionPage implements OnInit {
  ...
  getPosts(): void {
    this.loading = true;
    this.statusMessage = 'Consultando API...';

    this.apiService.getPosts().subscribe({
      next: (posts) => {
        this.posts = posts.slice(0, 6);
        this.appDataService.saveApiPosts(this.posts);
        this.statusMessage = 'Posts cargados desde JSONPlaceholder.';
        this.loading = false;
      },
      error: (error: Error) => {
  ...
  createPost(): void {
    ...
    this.apiService.createPost(post).subscribe({
      next: (createdPost) => {
        ...
        this.appDataService.saveApiPosts(this.posts);
        this.statusMessage = 'Post creado mediante POST.';
        this.loading = false;
      },
      error: (error: Error) => {
  ...
  updatePost(): void {
    ...
    this.apiService.updatePost(id, post).subscribe({
      next: (updatedPost) => {
        ...
        this.appDataService.saveApiPosts(this.posts);
        this.statusMessage = 'Post modificado mediante PUT.';
        this.loading = false;
      },
      error: (error: Error) => {
  ...
  deletePost(post: ApiPost): void {
    ...
    this.apiService.deletePost(id).subscribe({
      next: () => {
        ...
        this.appDataService.saveApiPosts(this.posts);
        this.statusMessage = 'Post eliminado mediante DELETE.';
        this.loading = false;
      },
      error: (error: Error) => {
